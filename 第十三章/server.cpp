...
参数 share_mem 指出共享内存的起始地址 */
int run_child( int idx, client_data* users, char* share_mem ){
    epoll_event events[ MAX_EVENT_NNUMBER ];
    ...
    while( !stop_child ){
        int number = epoll_wait( child_epollfd, events, MAX_EVENT_NNUMBER, -1 );
...
int main( int argc, char* argv[] ){
    ...
    close( shmfd );
    while( !stop_server ){
        int number = epoll_wait( epollfd, events, MAX_EVENT_NNUMBER, -1 );
        if( ( number < 0 ) && ( errno != EINTR ) ){
            printf( "epoll failure\n" );
            break;
        }
