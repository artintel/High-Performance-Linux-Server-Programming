...
/* 将 fd 上的 EPOLLIN 和 EPOLLET 事件注册到 epollfd 指示的 epoll 内核事件表中，参数 oneshot 指
定是否注册 fd 上的 EPOLLONESHOT 事件 */
...
int main( int argc, char* argv[] ){
    ...
    assert( epollfd != -1 );
    /* 注意，监听socket listenfd 上时不能注册 EPOLLONESHOT 事件上，否则应用程序只能处理一个客户连接
