...
/* 将文件描述符 fd 上的 EPOLLIN 注册到 epollfd 指示的 epoll 内核事件表中，参数 enable_et 指定是否对
fd 启用 ET 模式 */
...
void et( epoll_event* events, int number, int epollfd, int listenfd ){
    ...
    for( int i = 0; i < number; i++ ){
        ...
        if( sockfd == listenfd ){
            ...
            /* 这段代码不会被触发，所以我们循环读取数据，以确保把 socket 读缓存中的所有数据读出 */
            printf( "event trigger once\n" );
            while( 1 ){
                memset( buf, '\0', BUFFER_SIZE );
                int ret = recv( sockfd, buf, BUFFER_SIZE - 1, 0 );
...
int main( int argc, char* argv[] ){
    ...
    assert( epollfd != -1 );
    addfd( epollfd, listenfd, true );// listenfd监听这个文件操作符也被加入到了事件表！！！
    while( 1 ){
